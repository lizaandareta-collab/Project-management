...
/* Styling untuk Milestone Task berdasarkan kategori */
        ...
        <div class="modal-content">
            <form id="editRemarksForm" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                        <label class="form-label">Milestone Task</label>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Event listener untuk modal remarks
...
function f_autoload() {
        ...
        tbodyHtml += `
            <tr data-row-id="${t.id}">
               ...
               <!-- Actual Start -->
                ...
                <!-- Actual Duration -->
                <td>
                    ${generateEditableField('actual_duration', t.task_id, 'Actual Duration', t.actual_duration, t.actual_duration)}
                </td>

                <!-- Actual End (AUTO) -->
                <td>
                    ${t.actual_end ? format_date_local(t.actual_end) : ''}
                </td>
    ...
    function calculateKPIStatus(task) {
        // Jika belum mulai (tidak ada actual start) - tidak tampilkan indikator
        ...
        if (task.actual_start) {
            ...
            // Jika actual start ada tapi actual end belum ada (masih berjalan)
            return {
        ...
        // Default: tidak tampilkan indikator
        return {
            class: '',
            tooltip: 'Not Started - Belum dimulai'
        };
    ...
    function add_task(icon) {
        ...
        if (fieldType === 'responsible') {
            ...
            inputField = `
                <label class="form-label">${fieldLabel}</label>
                <input type="date" class="form-control" name="actual_start" value="${currentValue ? currentValue.split(' ')[0] : ''}" required>
            `;

        } else if (fieldType === 'actual_duration') {
            inputField = `
                <label class="form-label">${fieldLabel}</label>
                <input type="number" min="0" class="form-control" name="actual_duration" value="${currentValue ?? ''}" required>
            `;

        } else if (fieldType.includes('duration') || fieldType.includes('hour')) {
            inputField = `
                <label class="form-label">${fieldLabel}</label>
                <input type="number" min="0" step="1" class="form-control" name="${fieldType}" value="${currentValue || ''}" required>
            `;
    ...
    function openRemarksModal(taskId, milestoneTask, currentValue, fromStatusUpdate = false) {
        ...
        document.getElementById('activity_name').value = milestoneTask;
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    ...
                    if (item.docs && item.docs.length > 0) {
                        fileHtml = item.docs
                            .map(doc => {
                                return `<a href="/${doc.full_path}" target="_blank">${doc.client_name}</a>`;
                            })
                            .join('<br>');
                    }
            ...
            .then(res => {
                if (res.success) {
                    ...
                    if (cell) {
                        ...
                        if (fieldType.includes('date') && newValue) {
                            displayValue = format_date_local(newValue);
                        } else if (fieldType === 'actual_progress') {
                            displayValue = newValue + '%';
                        }
                        if (fieldType.includes('duration')) {
                            displayValue = newValue;
                        }
    ...
    function calculate_duration(row, changedField, newValue, heatTaskFromServer = null, planEndFromServer = null, actualEndFromServer = null) {
        ...
        const updateFieldValue = (fieldType, value, displayValue = null) => {
            const cell = row.querySelector(`[data-type="${fieldType}"]`);
        ...
        // === HITUNG PLAN END ===
        const planEndCell = row.querySelector('td:nth-child(8)');
        ...
        // === HITUNG ACTUAL END ===
        const actualEndCell = row.querySelector('td:nth-child(11)');
        if (actualEndFromServer) {
            actualEndCell.textContent = format_date_local(actualEndFromServer);
        } else {
            const actualStart = getFieldValue('actual_start');
            const actualDuration = getFieldValue('actual_duration');
            if (actualStart && actualDuration) {
                const actualEnd = working_day(actualStart, parseInt(actualDuration));
                actualEndCell.textContent = actualEnd;
            }
        }

        // === AUTO CALCULATE/UPDATE HEAT_TASK ===
        const planHour = getFieldValue('plan_hour');
        const actualDurationForHeat = getFieldValue('actual_duration');

        let heatTask = null;
        if (heatTaskFromServer !== null) {
            heatTask = heatTaskFromServer;
        } else if (planHour && actualDurationForHeat && parseInt(actualDurationForHeat) > 0) {
            heatTask = (parseFloat(planHour) / parseInt(actualDurationForHeat)).toFixed(1);
        }
        if (cells[17] && heatTask !== null) {
            cells[17].textContent = heatTask;
        }

        update_left_join(row, changedField, newValue);
